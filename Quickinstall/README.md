...
- `projectying / projectling` 走独立仓库路线；默认仓库为 `ProjectYing` / `ProjectLing`，可用 `AITERMUX_PROJECTYING_REPO`、`AITERMUX_PROJECTLING_REPO` 覆盖。
...
- ProjectLing 角色卡缓存会跟踪核心源码、Settings、主星/执行星角色状态；设置或更新返回后会自动失效，不继续展示旧双星状态。
...
- 打开 `PROJECT凌设置`：如果本地缺少 `projectling`，会从 `ProjectLing` clone
