...
- `projectying / projectling` 走独立仓库路线；默认仓库为 `projectying-termux` / `PROJECTling`，可用 `AITERMUX_PROJECTYING_REPO`、`AITERMUX_PROJECTLING_REPO` 覆盖。
...
- 打开 `PROJECT凌设置`：如果本地缺少 `projectling`，会从唯一公开仓库 `PROJECTling` clone
