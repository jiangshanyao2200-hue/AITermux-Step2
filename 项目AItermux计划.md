...
5 动画结束后，`~/.termux/motd.sh` 绘制静态赛博欢迎页头部并接管 Launcher 菜单（上下选择 / Enter 启动 / Esc 进 shell；启动项在当前页继续执行）→ `login` 读取 `~/.termux/shell`（当前固定到 `$PREFIX/bin/zsh`）后 `exec $SHELL -l`（在启动项退出或跳过菜单后进入普通 zsh）
...
- `~/.termux/motd.sh`：唯一开屏入口（随机播动画、超时兜底、日志、清屏、容错；动画结束后绘制静态赛博欢迎头）
- `~/.termux/shell`：登录 shell 选择器；需固定到 `$PREFIX/bin/zsh`，确保 `motd` 菜单之后落回统一的 zsh 环境
- `~/.zshrc`：保持普通 shell；Launcher 菜单已回收至 `~/.termux/motd.sh`
...
### B. 可选覆盖（体验/偏好）
- `~/.termux/termux.properties`：键位/软键盘等体验优化（不影响 AITermux 正常运行；Quickinstall 已内置模板并会覆盖写入）
  - `extra-keys`：将 `-` 替换为 `INS`（`\u001b[2~`），将 `+` 替换为 `END`（`\u001b[4~`）
  - 取消额外增加的 `F1~F8` 行：会压缩终端可视区域；若未来确实需要，可改用 `popup`（长按弹出）避免占用整行
...
3 `$PREFIX` 内文件（`$PREFIX/bin/login`、`$PREFIX/bin/tx11start`、`$PREFIX/etc/motd.sh`、`$PREFIX/etc/termux-login.sh`）一般不需要 root：只要脚本在 Termux 内运行即可写入（这些文件属于 Termux App 用户）；同时需确保 `$PREFIX/bin/zsh` 已安装，并把 `~/.termux/shell` 指到它
